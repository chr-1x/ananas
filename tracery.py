import json, random
from pineapple import PineappleBot, ConfigurationError, hourly, reply
...
class TraceryBot(PineappleBot):
    def init(self):
        self.root_symbol = "origin"
    def start(self):
        if not hasattr(self, "grammar_file"): raise ConfigurationError("TraceryBot requires a 'grammar_file'")
        if not hasattr(self, "root_symbol"): raise ConfigurationError("TraceryBot requires a 'root_symbol'")
        with open(self.grammar_file, "r") as f:
            if f: self._grammar = TraceryGrammar(json.load(f))
    ...
    def post(self, mention, user):
        ...
        self._mastodon.status_post("@{} {}".format(user["username"], 
                self._grammar.eval(self.root_symbol)),
                in_reply_to_id = mention["id"],
                visibility = mention["visibility"])

    @hourly()
    def post(self):
        self._mastodon.toot(self._grammar.eval(self.root_symbol))
